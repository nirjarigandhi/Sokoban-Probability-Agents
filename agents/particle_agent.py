...
class ParticleAgent(ProbabilityAgent):
  ...
  def listen(self, state):
    ...
    thougths = Counter(self._thoughts)
    for pos in echo_dist:
      if pos in thougths:
        count=1
        break
    ...
    updated_thougths = Counter()
    echo_dist = self._echo_grid.get_echo_distribution()
    for pos in self._valid_positions:
      for newPos, prob in echo_dist.items():
        if newPos in self._valid_positions:
          updated_thougths[newPos] += prob * thougths[newPos]
          
    if (sum(updated_thougths.values()) > 0):
      DistributionModel.normalize(updated_thougths)
    self._thoughts = updated_thougths
    # Write your code here
